...
当前已发布版本为 `v0.1.2`，版本 tag 推送后由 GitHub Actions 自动生成双端安装包。源码开发分支已实现本地任务、坚果云自动同步、离线接力、自建 Worker、双端任务通知和恢复备份主链路；新功能仍需生成下一版安装包并完成目标真机验收。进度见 [执行计划](docs/EXECUTION_PLAN.md)。
