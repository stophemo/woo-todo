...
<p align="center">
  ...
  <a href="https://github.com/stophemo/woo-todo/releases/tag/v0.1.23">下载 v0.1.23</a> ·
...
当前稳定版是 [`v0.1.23`](https://github.com/stophemo/woo-todo/releases/tag/v0.1.23)，提供 macOS ZIP、Android APK 与 Windows 免安装 ZIP。macOS ZIP 内是可直接运行的 `Woo Todo.app`，不是 DMG；Windows ZIP 内只有原生 `WooTodo.exe`，没有安装器。三端都坚持原生、轻量和本地优先。

| 平台 | 最适合的场景 | 状态 |
| --- | --- | --- |
| macOS 15+、Apple Silicon | 桌面层任务小组件、悬浮任务板、完整任务管理与统计 | `v0.1.23` 可下载 |
| Android 13+ | 睡前规划、任务提醒、今日 Widget 与移动查看 | `v0.1.23` 可下载 |
| Windows 10 build 19041+ / Windows 11（仅 x64） | 原生悬浮任务板、完整任务管理、统计与加密同步 | `v0.1.23` 可下载 |
...
| 平台 | 发布文件 | 使用提示 |
| --- | --- | --- |
| macOS | [Woo-Todo-v0.1.23-macos-arm64.zip](https://github.com/stophemo/woo-todo/releases/download/v0.1.23/Woo-Todo-v0.1.23-macos-arm64.zip) | ZIP 内是 `Woo Todo.app`；解压后可直接运行，也可移入“应用程序”。当前为 ad-hoc 签名且未经过 Apple 公证，首次打开可能需要在“隐私与安全性”中允许。 |
| Android | [Woo-Todo-v0.1.23-android.apk](https://github.com/stophemo/woo-todo/releases/download/v0.1.23/Woo-Todo-v0.1.23-android.apk) | 从系统文件管理器打开并允许本次来源安装。正式 APK 使用项目长期 Release 签名，可直接覆盖升级。 |
| Windows | [Woo-Todo-v0.1.23-windows-x64.zip](https://github.com/stophemo/woo-todo/releases/download/v0.1.23/Woo-Todo-v0.1.23-windows-x64.zip) | ZIP 内只有 `WooTodo.exe`；解压后直接运行，无需安装、管理员权限、.NET 或 Visual C++ Runtime。当前程序未签名，SmartScreen 可能提示来源未知。 |
| 完整性校验 | [SHA256SUMS.txt](https://github.com/stophemo/woo-todo/releases/download/v0.1.23/SHA256SUMS.txt) | 下载后用 `shasum -a 256 <文件>` 或系统等价工具核对。 |

三端的 `v0.1.22` 都可在应用内升级到 `v0.1.23`。macOS `v0.1.14` 因 Sparkle 框架打包路径错误而无法启动，仍需手动下载新 ZIP 并覆盖旧应用一次。覆盖更新会保留任务、设置和同步身份；不要先卸载或清除应用数据。
...
1. 解压 `Woo-Todo-v0.1.23-windows-x64.zip`，运行其中的 `WooTodo.exe`；若 SmartScreen 拦截，请先确认文件来自本仓库 Release 并核对校验和。
