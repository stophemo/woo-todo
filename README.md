...
<p align="center">
  ...
  <a href="https://github.com/stophemo/woo-todo/releases/tag/v0.1.27">下载 v0.1.27</a> ·
...
当前稳定版是 [`v0.1.27`](https://github.com/stophemo/woo-todo/releases/tag/v0.1.27)，正式提供 macOS ZIP 与 Android APK。Windows 同步提供独立的实验版免安装 ZIP，但不属于正式发布通道，也不保证能够正常启动或使用。

| 平台 | 最适合的场景 | 状态 |
| --- | --- | --- |
| macOS 15+、Apple Silicon | 桌面层任务小组件、悬浮任务板、完整任务管理与统计 | `v0.1.27` 正式版 |
| Android 13+ | 睡前规划、任务提醒、今日 Widget 与移动查看 | `v0.1.27` 正式版 |
| Windows 10 build 19041+ / Windows 11（仅 x64） | 原生悬浮任务板、完整任务管理、统计与加密同步 | `v0.1.27` 实验版（不保证可用） |
...
| 平台 | 发布文件 | 使用提示 |
| --- | --- | --- |
| macOS | [Woo-Todo-v0.1.27-macos-arm64.zip](https://github.com/stophemo/woo-todo/releases/download/v0.1.27/Woo-Todo-v0.1.27-macos-arm64.zip) | ZIP 内是 `Woo Todo.app`；解压后可直接运行，也可移入“应用程序”。当前为 ad-hoc 签名且未经过 Apple 公证，首次打开可能需要在“隐私与安全性”中允许。 |
| Android | [Woo-Todo-v0.1.27-android.apk](https://github.com/stophemo/woo-todo/releases/download/v0.1.27/Woo-Todo-v0.1.27-android.apk) | 从系统文件管理器打开并允许本次来源安装。正式 APK 使用项目长期 Release 签名，可直接覆盖升级。 |
| Windows 实验版 | [Woo-Todo-v0.1.27-windows-x64-experimental.zip](https://github.com/stophemo/woo-todo/releases/download/windows-v0.1.27-experimental/Woo-Todo-v0.1.27-windows-x64-experimental.zip) | **不保证可用。** ZIP 内含 `WooTodo.exe` 与 `WINDOWS-EXPERIMENTAL.txt`；当前程序未签名，SmartScreen 可能提示来源未知。请先备份重要数据。 |
| 正式版校验 | [SHA256SUMS.txt](https://github.com/stophemo/woo-todo/releases/download/v0.1.27/SHA256SUMS.txt) | 校验 macOS 与 Android 正式包。 |
| Windows 实验版校验 | [SHA256SUMS-windows-experimental.txt](https://github.com/stophemo/woo-todo/releases/download/windows-v0.1.27-experimental/SHA256SUMS-windows-experimental.txt) | 校验独立 Windows 实验包。 |

`v0.1.27` 在 macOS 顶部和 Android Widget 显示农历日期、节气或节日注释，Android Widget 新增背景透明度设置；macOS WebDAV 完整配置改为本机加密文件保存并自动回填。macOS 与 Android 的 `v0.1.26` 可在应用内升级；Windows 仅提供独立实验包，不参与正式版自动更新。
...
1. Windows 当前仅提供 `Woo-Todo-v0.1.27-windows-x64-experimental.zip` 实验包；它不保证可用。备份数据后解压并运行 `WooTodo.exe`，若 SmartScreen 拦截，请核对独立实验版校验和。
