...
/**
 * 桌面端近旁配置 Android 的 WebDAV 深链。该链接等同于认证凭据和同步密钥，不能写入日志或持久化。
 */
data class WebDavSetupLink(
    ...
    init {
        require(WebDavEndpointPolicy.isAllowed(endpoint)) { "WebDAV 服务地址无效" }
        require(WebDavCredentialPolicy.isValidUsername(username)) { "WebDAV 账号无效" }
        require(WebDavCredentialPolicy.isValidAppPassword(appPassword)) { "WebDAV 应用密码无效" }
    ...
    fun toUri(): String = buildString {
        ...
        append("v=2")
        append('&').append(query("endpoint", endpoint))
    ...
    override fun equals(other: Any?): Boolean = other is WebDavSetupLink &&
        endpoint == other.endpoint && username == other.username && appPassword == other.appPassword &&
        vaultId == other.vaultId && vaultKey.contentEquals(other.vaultKey)

    override fun hashCode(): Int = 31 * listOf(endpoint, username, appPassword, vaultId).hashCode() +
        vaultKey.contentHashCode()

    override fun toString(): String =
        "WebDavSetupLink(endpoint=<已隐藏>, username=<已隐藏>, appPassword=<已隐藏>, vaultId=<已隐藏>, vaultKey=<已隐藏>)"
    ...
    companion object {
        ...
        fun parse(source: String): WebDavSetupLink {
            val uri = try {
                ...
                throw IllegalArgumentException("WebDAV 配置链接不是有效 URI", error)
            }
            require(
            ...
            uri.rawQuery!!.split('&').forEach { part ->
                require(part.isNotEmpty()) { "WebDAV 配置链接包含空字段" }
                val separator = part.indexOf('=')
                require(separator > 0) { "WebDAV 配置链接字段格式无效" }
            ...
            require(values.keys == requiredKeys) { "WebDAV 配置链接字段不完整" }
            require(values.getValue("v") == "2") { "WebDAV 配置链接版本不支持" }
            return WebDavSetupLink(
                endpoint = values.getValue("endpoint"),
                ...
                vaultKey = try {
                    ...
                    throw IllegalArgumentException("WebDAV 配置链接同步密钥无效", error)
                },
            )
        }
        ...
        private fun decode(value: String): String = try {
            ...
            throw IllegalArgumentException("WebDAV 配置链接编码无效", error)
        }
    }
}
