...
离线使用与跨端同步是两个阶段：没有网络时，每个客户端仍在同一 SQLite 事务中更新任务与持久化 outbox，UI 立即完成操作；联网后才发送积压变更。两台设备不必同时在线，例如手机今晚上传后可以断网，Mac 明早联网再拉取。若两台设备都没有任何可用网络，它们仍可各自使用，但在物理上无法互相获得新数据；v1 不实现同一局域网内的设备发现和点对点直连。
