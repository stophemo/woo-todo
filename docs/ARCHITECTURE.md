...
用户先通过坚果云官方客户端登录账号，并在网页安全设置中生成应用密码。推荐先在 Mac 配置；连接成功后二维码默认隐藏，用户显式点击显示，再由 Android Woo Todo 内置扫码器解析、确认并保存。二维码严格包含 `username`、`appPassword`、`vaultId` 和 `vaultKey`，不含固定 WebDAV 端点或 `deviceId`，Android 保存时生成本机独立 `deviceId` 并立即同步。该二维码等同完整敏感凭据，只能近旁展示，用完立即隐藏，不进入 Woo Todo 服务或日志。Woo Todo 内置 WebDAV 客户端直接连接坚果云，不经过网页上传，也不上传 SQLite 或整份 JSON，而是为每个本地 outbox 操作写入一个不可变密文对象：
