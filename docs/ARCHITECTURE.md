...
- Carbon Hot Key：无需常驻事件轮询的可自定义全局快捷键，默认 `⇧⌥N` 快速新增、`⇧⌥L` 显隐任务板、`⇧⌥T` 置顶、`⌃⌥Space` 穿透。
...
默认数据通道仍是本地 SQLite；跨端自动同步优先使用坚果云 WebDAV。完全离线时可用用户主动导出、传递并合并的加密 `.wootodo` 接力包。接力不复制同步身份，在目标 SQLite 单事务中按 tombstone、结算终态、`updatedAt` 和稳定内容指纹合并；重复导入幂等。

### 4.1 坚果云 WebDAV 自动同步

坚果云模式不上传 SQLite 或整份 JSON，而是为每个本地 outbox 操作写入一个不可变密文对象：
