...
2. `Control + Option + N` 从任意应用打开快速新增框，`Enter` 新增今日主线一次性任务，`Esc` 取消；
...
6. 在“任务详情与统计”管理本周、本月、闲时、历史、统计、离线接力、恢复备份和可选同步。
...
4. “更多”中设置睡前提醒、导入/导出离线接力包和恢复备份，“统计”查看历史与履约趋势；
...
只有确实需要跨地点自动同步时，才自行部署 Cloudflare Workers + D1。夸克网盘可保存加密文件，但不能充当在线增量同步服务。
...
- macOS 快速新增、悬浮、置顶、毛玻璃、穿透和两个全局快捷键连续运行 24 小时；
