...
- 正式版高于当前版本时进入平台更新流程；等于或低于当前版本时，手动检查显示“已是最新”
- 草稿、预发布、非三段式 tag、错误仓库 URL、错误资产名与外部下载地址均拒绝
- 自动检查最长每 24 小时一次；DNS、超时或 HTTP 失败不影响本地任务、同步、Widget 或提醒
- macOS Release 实际嵌入 Sparkle framework、Updater、Autoupdate 与 XPC 服务；appcast enclosure 指向当前 ZIP 并包含可由固定公钥验证的 Ed25519 签名
- Android 下载后拒绝错误包名、错误版本、未递增 `versionCode`、损坏包和不同签名 APK；Activity 销毁或用户取消时停止下载并清理临时文件
- Android 同包名、同签名覆盖升级保留 SQLite、Keystore 与同步凭据；卸载、清除数据或更换签名才进入重新配置流程
- Windows helper 等待主进程退出，复核 SHA-256 和 ZIP 只含根目录 `WooTodo.exe`，替换后重启；新 EXE 启动失败时恢复备份，数据目录始终不参与替换
