...
- 状态：已接受
- 日期：2026-07-26
- 修订：ADR-0001、ADR-0003 中“领域逻辑分别维护”的部分；Windows 接入方式后来由 ADR-0003 的 2026-07-27 修订更新

## 背景

Woo Todo 同时要求轻量、高性能、交互优雅，以及 macOS、Android、Windows 的可靠系统通知。AppKit/SwiftUI、Android Views/RemoteViews、Win32 能直接覆盖悬浮窗口、Widget、托盘、全局快捷键和通知生命周期；但继续按平台各写一份周期、重复、结算和统计逻辑，会扩大行为漂移与测试成本。
...
- Rust 外壳可直接依赖核心 crate；跨语言外壳通过窄 C ABI 暴露 UTF-8 JSON 请求/响应。共享核心不持有平台 UI 对象。
- macOS 继续使用 Swift + AppKit/SwiftUI，Android 继续使用 Kotlin + Views/RemoteViews，Windows 使用 Rust + `windows-rs`/Win32。
...
1. Windows 早期以 C# 薄适配层验证共享核心与系统通知链路，随后按 ADR-0003 修订为 Rust + Win32，并删除该 FFI 与动态库层。
...
领域规则只维护一份，三端仍保留直接使用系统能力的交互质量和后台可靠性。代价是发布流程需要为各目标架构构建 Rust 产物；跨语言接入仍必须保持 FFI 内存所有权、JSON 字段和错误语义稳定。Windows 直接链接 crate，不承担这层边界；macOS 与 Android 完成渐进迁移前还需运行双实现一致性测试。
