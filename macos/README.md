...
- 菜单栏恢复入口；`Control + Option + Space` 切换鼠标穿透/交互状态。
- `Control + Option + N` 从任意应用唤起紧凑新增框，`Enter` 写入今日主线任务，`Esc` 取消。
- 今日任务新增、编辑、勾选完成、删除及同级排序基础。
- 菜单栏“任务详情与统计…”按需打开管理窗口，关闭后释放窗口资源。
- 管理窗口包含今日、本周、本月、闲时、历史和统计六个任务分区，并提供独立同步分区。
- 管理窗口提供同步分区，可创建首个加密同步空间、显示 Android 配对二维码与六位核对码。
- 无传统账号的设备绑定；同步凭据与 vault key 仅保存在本机 Keychain，或用户主动导出的加密备份中。
- SQLite 加密 outbox、增量 cursor、幂等远端应用和 tombstone 删除同步。
- 启动、本地修改、系统唤醒、网络恢复和 15 分钟低频兜底触发后台同步；失败不阻塞本地操作。
- 已绑定设备列表和远端撤销；当前设备不能撤销自身。
- 完整编辑器保持一句话任务，支持时间范围、目标周期、级别及同周期重复。
...
应用安装后默认纯离线，不需要部署服务。跨端可主动交换加密接力包；只有需要跨地点自动同步时，才按仓库文档自行部署 Cloudflare Workers + D1，并在“同步”页填写 HTTPS 根地址和部署者提供的创建空间邀请码。GitHub Release 产物使用 ad-hoc 签名且没有 Apple 公证；macOS 端也不承担手机睡前提醒。
