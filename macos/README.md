...
- 六项可自定义全局快捷键：默认 `Shift + Option + 1` 快速新增、`Shift + Option + 2` 显隐任务板、`Shift + Option + 3` 置顶、`Shift + Option + 4` 穿透、`Shift + Option + ↑/↓` 增减透明度；菜单栏始终保留恢复入口。
...
- 同步页支持 PBKDF2 + AES-256-GCM `.wootodo` 加密备份；恢复仅允许空白安装。
