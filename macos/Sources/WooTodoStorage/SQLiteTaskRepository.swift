...
public final class SQLiteTaskRepository: TaskRepository, SyncOutbox, SyncLocalApplying,
    ...
    /// 将另一台设备导出的任务快照合并到当前本地库，不恢复或复制同步身份。
    /// 整个计划与写入位于同一 SQLite 事务，重复导入不会产生额外变更。
    public func mergeOfflineRelayPayloads(
        _ payloads: [WireTaskPayload],
        tombstones: [WireTombstonePayload] = []
    ) throws -> OfflineRelayMergeResult {
        try withLock {
            let local = try makeBackupContents()
            let plan = try OfflineRelayMergePolicy.plan(
                localTasks: local.tasks,
                localTombstones: local.tombstones,
                incomingTasks: payloads,
                incomingTombstones: tombstones
            )

            try execute("BEGIN IMMEDIATE TRANSACTION")
            do {
                for payload in plan.tasksToUpsert {
                    let task = try task(from: payload)
                    let previous = try storedTask(id: task.id)
                    try upsertTaskRow(task)
                    let entityID = canonicalEntityID(task.id.uuidString)
                    if let configuration = syncConfiguration {
                        try enqueueLocalTask(
                            task,
                            kind: operationKind(previous: previous, current: task),
                            configuration: configuration
                        )
                    } else if hasPersistedSyncIdentity {
                        try recordDeferredUpsert(entityID: entityID)
                ...
                for tombstone in plan.tombstonesToApply {
                    let entityID = canonicalEntityID(tombstone.id)
                    try deleteTaskRow(entityID: entityID)
                    try removeDeferredUpsert(entityID: entityID)
                    if let configuration = syncConfiguration {
                        try enqueueLocalTombstone(
                            entityID: entityID,
                            deletedAtMilliseconds: tombstone.deletedAt,
                            configuration: configuration
                        )
                    } else {
                        try recordDeferredDeletion(
            ...
            return OfflineRelayMergeResult(
                mergedTaskCount: plan.tasksToUpsert.count,
                mergedTombstoneCount: plan.tombstonesToApply.count,
